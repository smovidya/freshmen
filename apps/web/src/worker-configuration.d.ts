/* eslint-disable */
// Hand-maintained (not `wrangler types`): provides the `cloudflare:workers` ambient
// module and runtime types (D1Database, etc.) needed to typecheck `import type { auth }
// from '@vidyafreshmen/auth'` in src/lib/auth/client.ts (better-auth's client needs the
// server auth's type for full session inference). Don't regenerate this from
// apps/api/wrangler.jsonc — that pulls in unrelated bindings whose type-only imports
// (e.g. GAME_REGION_HANDLER -> apps/api/src/index.ts) drag that whole Worker's source
// into this package's compile scope.
