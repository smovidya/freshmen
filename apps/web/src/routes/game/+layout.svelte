...
{#if qte.visible}
	<button
		type="button"
		onclick={() => qte.claim()}
		class="animate-qte-slide-down fixed inset-x-0 top-0 z-[10000] flex w-full items-center gap-3 bg-gradient-to-r from-[#fdf886] to-[#ffe066] px-4 py-3 text-left text-[#7a5a0a] shadow-lg"
	>
		...
		<span class="h-1.5 w-16 shrink-0 overflow-hidden rounded-full bg-black/10">
			<span
				...
				style="width: {(qte.remainingMs / VISIBLE_WINDOW_MS) *
					100}%; transition: width 0.1s linear;"
			></span>
		</span>
	</button>
{/if}
