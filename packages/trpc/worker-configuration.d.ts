...
// module and runtime types (D1Database, etc.) needed to typecheck this package's
// dependencies (@vidyafreshmen/auth, @vidyafreshmen/db) standalone. Don't regenerate
// this from apps/api/wrangler.jsonc — that pulls in unrelated bindings whose
// type-only imports (e.g. GAME_REGION_HANDLER -> apps/api/src/index.ts) drag that
// whole Worker's source into this package's compile scope.
